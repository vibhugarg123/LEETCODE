...
public class ValidPalindrome125 {
    public boolean isPalindrome(String s) {
        for (int start = 0, end = s.length() - 1; start <= s.length() - 1 && end >= 0 && start < end; ) {
            ...
            if (start <= s.length() - 1 && end >= 0 && Character.toLowerCase(s.charAt(start)) != Character.toLowerCase(s.charAt(end)))
                return false;
    ...
    public static void main(String[] args) {
        String[] s = {"A man, a plan, a canal: Panama", "race a car", " ", "hoh"};
        for (String str : s) {
            System.out.println(new ValidPalindrome125().isPalindrome(str));
        }
    }
}